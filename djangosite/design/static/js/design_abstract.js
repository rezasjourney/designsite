...
function setupCanvas() {
	...
	if (!("background_elem"  in design))
	{
		...
		back_elem.strokeEnabled = true;
		back_elem.strokeWidth = 2;
...
function setupImageElement(elem, image, layer, sugg_layer, selected) {
	...
	if (!$('#canvas').data("hideContent")) {
		img = new Kinetic.Image({
			...
			name : elem.id,
			strokeEnabled : false, //$('#canvas').data("hideContent"),
			stroke : 'Black',
			strokeWidth : 2,
			//lineJoin : 'round',
			//dashArray : [7, 5],
			...
			callback : function(text_img) {
				...
				var add_background = function(pixels) {
					...
					for (var i = 0; i < d.length; i += 4) {
						...
						//|| (i%(4*text_img.width)> 4*text_img.width-8 )
						//if ((i<2*text_img.width-50) || (i>d.length-2*text_img.width+50) || (i%(2*text_img.width-54)<4) || (i%(2*text_img.width-54)> 2*text_img.width -64   )
						//{
						//	d[i] =0
						//	d[i+1] =0
						//	d[i+2] =0
...
function selectAllElements() {
	...
	$.each(elements, function(i, e) {
		e.img.attrs.strokeEnabled = false;//true;
		$.each(e.anchors, function(i, a) {
			a.hide();
		});
		e.selected = true;
	...
	$.each(alignments, function(index, curr_alignment) {
		...
		if ($('#canvas').data("hideContent"))
		{
			...
			text = new Kinetic.Text({
				...
				width : Math.round(elem.height * elem.aspect_ratio),
				fontSize : 20,
				fontFamily : 'Calibri',
				align : 'center'
			});
	...
	text.toImage({
		...
		callback : function(img) {
			...
			if ($('#canvas').data("hideContent")) {
				...
				add_background = function(pixels) {
					...
					for (var i = 0; i < d.length; i += 4) {
						...
						//if ((i<4*img.width-20) || (i>d.length-4*img.width+20) || (i%(4*img.width-20)<4 ) || (i%(4*img.width-20)> 4*img.width-28 ))
						//{
						//	d[i] =0
						//	d[i+1] =0
						//	d[i+2] =0
						...
						var alpha = (d[i + 3] / 255)
							...
							d[i] += alpha * d[i] + (1 - alpha) * 109;
							d[i + 1] += alpha * d[i + 1] + (1 - alpha) * 229;
							d[i + 2] += alpha * d[i + 2] + (1 - alpha) * 114;
						//}
						d[i + 3] = 255;
					}
					return pixels;
			...
			var text_img = new Kinetic.Image({
				...
				strokeEnabled : $('#canvas').data("hideContent"),
				stroke : 'Black',
				strokeWidth : 2,
				lineJoin : 'round',
				//dashArray : [7, 5],
...
function selectElement(elem, multiple) {
	...
	if (elem.type!='background')
		elem.img.attrs.strokeEnabled = false;//true;
	...
	if (layout != new_layout) {
		...
		for (var i = 0; i < design.elements.length; i++) {
			...
			if ((render_info!= undefined) || (playback!=undefined)){
				...
				elem.img.attrs.strokeEnabled=false//selected
...
function startSuggestions(run_offset) {
	...
	if (getURLParameter("brainstorm") == 1){
		run_types=['gallery']
		console.log("do gallery suggestions")
	}
...
function moveToNextDesign()
{
	...
	console.log($('#canvas').data("designTime"))
