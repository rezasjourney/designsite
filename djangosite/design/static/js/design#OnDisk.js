	...
	$.each(design.elements, function(i, elem) {
		...
		if ($('#canvas').data("randomizeInit"))
		{
			...
			if (elem.type == 'graphic') 
				...
				new_height = 20 * elem.text.split("\n").length
			
	
			elem.height = new_height
			elem.width = elem.height * elem.aspect_ratio
			
			var new_x = Math.random() * (design.width - elem.width)
			var new_y = Math.random() * (design.height - elem.height)
...
function allowUpdates() {
	...
	if ((sug_check==undefined) || (sug_check==-1))
	{
		...
		if ($('#canvas').data("automaticUpdate"))
			...
			suggestion_check=setInterval(checkForSuggestions, 300);	
...
function setupLockingCallbacks(elem, layer) {
	...
	elem.unlock_img.on("click tap", function(evt) {
		...
		$('#canvas').data("user_input_log").push("St-"+String(elem.id)+"-twk")
		$('#canvas').data("status_log").push("St-"+String(elem.id)+"-twk")
	...
	elem.tweakable_img.on("click tap", function(evt) {
		...
		$('#canvas').data("user_input_log").push("St-"+String(elem.id)+"-lk" )
		$('#canvas').data("status_log").push("St-"+String(elem.id)+"-lk" )
		sendCurrentLayout('State Change')
	});
	elem.lock_img.on("click tap", function(evt) {
		setElementState(elem.id, 'unlocked')
		
		$('#canvas').data("user_input_log").push("St-"+String(elem.id)+"-ulk")
		$('#canvas').data("status_log").push("St-"+String(elem.id)+"-ulk")
		sendCurrentLayout('State Change')
	});
...
function setAlternate(elem, alt_elem) {
	...
	else if ((alt_elem.fixed_amount >0) && (alt_elem.fixed_amount <1))
...
function viewLayout(layout_type, layout_num) {
	...
	if ((layout_num < 0) && $('#canvas').data("preview") > -1) {
		...
		var image = $('#canvas').data("images")['checkmark'];
		var sugg_stage= $('#canvas').data("sugg_stage")
		
		//var sugg_stage.attrs.width / 3
...
function selectElement(elem, multiple) {
	...
	/*
	 ...
	 {
	 var pos =children[i].getAbsolutePosition()
	 console.log('element '+ String(children[i].attrs.name) + " has position "+ pos.x + " "+pos.y)
...
function sendCurrentLayout(user_input, reset) {
...
function getLayoutDiff(layout1, layout2) {
	...
	var num_elements = parseInt(elements1[3])

	if (num_elements != parseInt(elements2[3]))
		return 10000;
...
function addLayoutToStack(new_layout,stack_type) {
	...
	//console.log("addLayoutToStack "+stack_type)
	...
	/*
	if ((stack.length>0) && (getLayoutDiff(stack[stack.length-1][0],new_layout)<3))
	{
		return;
	}
	*/
	...
	if (layout != new_layout) {
		...
		for (var i = 0; i < design.elements.length; i++) {
			...
			if (elem.selected){
				...
				if ((Math.round(elem.height)!=new_height) || (Math.round(elem.x)!=new_x) || (Math.round(elem.y)!=new_y) || (new_alt_id!= elem.alternate_id) || (new_align_id ==elem.align_type))
					continue
...
function loadDesignFile(designName) {
	...
	var jsonLoad = $.getJSON(sprintf('/design/static/designs/%s.json', designName), function(design) {
		...
		//sources['checkmark'] = '/design/static/img/greencheck-transparent.png'
...
function moveToNextDesign()
{
	...
	if (curr_design_idx+1==sequence.length)
	{
		...
		$('#suggestion_status').text("HIT complete. Please submit after providing suggestions/comments.")
		$('#suggestion_status').css("background-color",'#5F5')
    ...
    for(var i = 0, l = fonts.length; i < l; ++i) {
        (function(font) {
            ...
            function checkFont() {
            	...
            	console.log("loaded fonts "+loadedFonts+ " array len "+fonts.length)
